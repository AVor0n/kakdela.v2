package ru.hh.kakdela.v2.constants;

public class ConstraintMessages {

  public static final String FIELD_SHOULD_NOT_BE_EMPTY = "Поле не должно быть пустым";
  public static final String FIELD_FORMAT_VIOLATED =
      "Содержимое не удовлетворяет требуемому формату";
  public static final String TEXT_VALUE_UPPER_LENGTH_LIMIT_VIOLATED =
      "Длина содержимого в символах не должна превышать ";
  public static final String PASSWORD_CONFIRMATION_NEEDED =
      "Операция должна быть подтверждена вводом текущего пароля";
  public static final String SERIAL_NUMBER_SHOULD_BE_POSITIVE
      = "Порядковый номер не может быть меньше 1";
  public static final String SERIAL_NUMBER_UPPER_LIMIT_VIOLATED
      = "Порядковый номер не должен быть больше ";
}
