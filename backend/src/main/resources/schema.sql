...
CREATE TABLE survey_subscription (
    survey_id uuid REFERENCES survey (id) ON DELETE CASCADE NOT NULL,
    account_id uuid REFERENCES account (id) ON DELETE CASCADE NOT NULL,
    PRIMARY KEY (survey_id, account_id)
);

CREATE INDEX idx_survey_subscription_survey 
ON survey_subscription (survey_id);
CREATE INDEX idx_survey_subscription_account
ON survey_subscription (account_id);
